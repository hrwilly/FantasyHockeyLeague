...
def build_roster_display(team_name):
    ...
    team_players = players[players["drafted_by"] == team_name]
    roster_display = []
...
# --- Free agents ---
def get_free_agents():
    return players[players["drafted_by"].isna()]

st.subheader("Available Free Agents")
free_agents = get_free_agents()
st.dataframe(free_agents[["Name", "Pos.", "team", "Ht.", "Wt."]],
...
if st.button("Add & Drop Player"):
    if not st.session_state.add_player or not st.session_state.drop_player:
        ...
        if current_count >= max_allowed:
            ...
            # Perform add/drop
            players.loc[players["Name"] == add_name, "drafted_by"] = my_team_name
            players.loc[players["Name"] == drop_name, "drafted_by"] = None
