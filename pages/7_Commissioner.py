...
def get_current_data(team):
    ...
    offense.loc['Cole Eiserman, So.'] = [0] * len(offense.columns)
    goalies.loc['Mikhail Yegorov, So.'] = [0] * len(goalies.columns)
...
if st.button("🏁 Run Weekly Scoring"):
    ...
    for i in range(len(coll_teams)):
        team_points = get_current_data(coll_teams['Name'][i][:-1])
        current_cum = pd.concat([current_cum, team_points])

    last_week = pd.read_csv('data/last_week_stats.csv')
        
    # Step 1: Compute weekly stats
    weekly_stats = current_cum.copy()
    if not last_week.empty:
        weekly_stats.iloc[:, 1:] = current_cum.iloc[:, 1:] - last_week.iloc[:, 1:].fillna(0)
    ...
    st.dataframe(weekly_scored.head(10))